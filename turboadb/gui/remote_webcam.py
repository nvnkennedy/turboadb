...
def _smb_push(host, login, password, local_ffmpeg, log=None):
    ...
    subprocess.run(
        ["net", "use", share, "/delete", "/y"], capture_output=True, creationflags=_NO_WINDOW
    )
    r = subprocess.run(
        ["net", "use", share, f"/user:{login}", "*"],
        input=password + "\r\n",
        capture_output=True,
        text=True,
        timeout=40,
...
def start_remote_stream(
    ...
    *,
    ...
    cmd_line = (
        f'"{ffmpeg}" -hide_banner -loglevel error -f dshow -rtbufsize 64M '
    ...
    script = (
        _kill_by_port_ps(stream_port) + "Start-Sleep -Milliseconds 500\n"
...
def stop_remote_stream(
    ...
    if pid:
        script += (
            f"try {{ Stop-Process -Id {int(pid)} -Force "
