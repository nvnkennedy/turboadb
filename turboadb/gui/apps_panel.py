...
class AppsPanel(QWidget):
    ...
    def __init__(self, handler, automotive=False, parent=None):
        ...
        ref.setProperty("role", "ghost")
        ref.clicked.connect(self.refresh)
        top.addWidget(QLabel("Apps:"))
        ...
        for label, slot, role in (
            ("Install APK(s)…", self._install, "ok"),
    ...
    def refresh(self):
        ...
        job = _Job(
            lambda: self.handler.list_packages(third_party=third, safe=False), emit_list=True
        )
    ...
    def _install(self):
        ...
        if len(files) > 1:
            ...
            def fn():
                return self.handler.install_multiple(files, grant_perms=True, safe=False)
        else:

            def fn():
                return self.handler.install(files[0], grant_perms=True, safe=False)
    ...
    def _uninstall(self):
        ...
        if QMessageBox.question(self, "Uninstall", f"Uninstall {pkg}?") == QMessageBox.Yes:
            self._do(lambda: self.handler.uninstall(pkg, safe=False), "uninstall", refresh=True)
    ...
    def _do(self, fn, label, refresh=False):
        job = _Job(fn)
        job.done.connect(
            lambda m: (self.log.emit(f"[OK] {label}: {m}"), self.refresh() if refresh else None)
        )
