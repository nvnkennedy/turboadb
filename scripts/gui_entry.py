...
def _selftest_winrm():
    ...
    for m in ("winrm", "requests", "requests_ntlm", "spnego", "xmltodict", "cryptography"):
        try:
    ...
    try:
        ...
        bad.append(sess)
    out = "WINRM-SELFTEST: " + (
        "ALL OK | " + sess if not bad else "PROBLEMS -> " + " || ".join(bad)
    )
    try:
        d = os.path.join(os.path.expanduser("~"), ".turboadb")
        os.makedirs(d, exist_ok=True)
        with open(os.path.join(d, "winrm-selftest.txt"), "w", encoding="utf-8") as fh:
            fh.write(out + "\n")
    except Exception:
        pass
